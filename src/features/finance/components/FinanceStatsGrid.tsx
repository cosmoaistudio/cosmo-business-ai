import {
  ArrowDownCircle,
  ArrowUpCircle,
  Scale,
  Wallet,
} from "lucide-react";
import { formatCurrency } from "@/lib/format";
import type { FinanceStats } from "../types/finance";

interface FinanceStatsGridProps {
  stats: FinanceStats;
  loading?: boolean;
}

export default function FinanceStatsGrid({
  stats,
  loading = false,
}: FinanceStatsGridProps) {
  const items = [
    {
      title: "Entradas",
      value: loading ? "..." : formatCurrency(stats.totalIncome),
      subtitle: loading ? "" : `${formatCurrency(stats.incomeToday)} hoje`,
      icon: ArrowUpCircle,
      color: "bg-emerald-500",
    },
    {
      title: "Saídas / Despesas",
      value: loading ? "..." : formatCurrency(stats.totalExpenses),
      subtitle: loading ? "" : `${formatCurrency(stats.expensesToday)} hoje`,
      icon: ArrowDownCircle,
      color: "bg-red-500",
    },
    {
      title: "Lucro",
      value: loading ? "..." : formatCurrency(stats.profit),
      subtitle: loading ? "" : `${formatCurrency(stats.profitToday)} hoje`,
      icon: Scale,
      color: "bg-blue-500",
    },
    {
      title: "Saldo",
      value: loading ? "..." : formatCurrency(stats.balance),
      subtitle: loading ? "" : `${stats.transactionCount} lançamentos`,
      icon: Wallet,
      color: "bg-violet-500",
    },
  ];

  return (
    <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
      {items.map((item) => {
        const Icon = item.icon;

        return (
          <div
            key={item.title}
            className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
          >
            <div className="flex items-center justify-between">
              <div
                className={`flex h-14 w-14 items-center justify-center rounded-2xl ${item.color} text-white`}
              >
                <Icon size={26} />
              </div>
            </div>

            <h3 className="mt-6 text-sm text-slate-500">{item.title}</h3>
            <p className="mt-2 text-3xl font-black text-slate-900">
              {item.value}
            </p>
            {item.subtitle && (
              <p className="mt-2 text-sm font-medium text-slate-500">
                {item.subtitle}
              </p>
            )}
          </div>
        );
      })}
    </div>
  );
}
